...
interface SongData { id: string; name: string; audio_path: string; genre?: string; key?: string; }
...
const AudioEditor: React.FC<AudioEditorProps> = ({ songId, recordingId, onSave, onCancel }) =>
{
  ...
  const [voiceTrack, setVoiceTrack] = useState<AudioTrack>({ buffer: null, volume: 1, name: 'Gravação de Voz' });
  ...
  const loadMusicTrack = useCallback(async (audioPath: string) =>
  {
    ...
    try
    {
      ...
      setMusicTrack(prev => ({ ...prev, buffer, name: songData?.name || 'Música' }));

      setPosition({ current: 0, duration: buffer.duration });
  ...
  }, [audioContext, songData?.name]);

  const loadRecordingTrack = useCallback(async (audioPath: string) =>
  {
    ...
    try
    {
      ...
      const segment: RecordingSegment = { buffer: decoded, startTime: 0, endTime: musicTrack.buffer.duration };
  ...
  }, [audioContext, musicTrack.buffer]);
        ...
        if (songId)
        {
          ...
          if (songResponse.ok)
          {
            ...
            if (song)
            {
              ...
              if (musicBuffer && Number(currentRecordingId) > 0)
              {
                ...
                if (recordingResponse.ok)
                {
                  ...
                  if (recording && recording[0] && recording[0].audio_path) await loadRecordingTrack(recording[0].audio_path);
                }
              }
            }
          }
    ...
    try
    {
      ...
      mediaRecorder.onstop = async () =>
      {
        try
        {
          ...
          {
            const filteredSegments = prev.filter(segment => segment.endTime <= newSegment.startTime || segment.startTime >= newSegment.endTime);
            return [...filteredSegments, newSegment].sort((a, b) => a.startTime - b.startTime);
          });
  ...
  const saveAudio = async () =>
  {
    ...
    try
    {
      ...
      formData.append('name', `Gravação ${new Date().toLocaleString()}`);
        ...
        <div className="fixed top-0 left-0 right-0 bg-stone-900 bg-opacity-95 backdrop-blur-sm shadow-md z-50 p-4 flex justify-start items-center gap-8 text-white font-mono text-sm select-none">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-semibold">
              {isLoading ? 'Carregando...' : (songData?.name || 'Nenhuma música carregada')}
            </h1>
            <div className="text-stone-200 flex gap-4">
              <span>{songData?.genre || 'Gênero não especificado'}</span>
              <span>•</span>
              <span>Duração: {formatDuration(position.duration)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative bg-stone-800 p-6 pb-0 rounded-t-xl mt-24">
          ...
          <div className="grid grid-cols-1">
            <div>
              ...
              <canvas ref={musicCanvasRef} width={1000} height={100} onMouseDown={(e) => handleCanvasSeek(e)}
            ...
            <div>
              ...
              <div className="relative">
                <canvas ref={voiceCanvasRef} width={1000} height={100} onMouseDown={(e) => handleCanvasSeek(e)}
                  className="w-full h-30 mb-2 bg-transparent cursor-pointer"/>
                <h3 className="text-base text-stone-200 pb-2 border-b border-stone-600">
                  Vocal
                </h3>
